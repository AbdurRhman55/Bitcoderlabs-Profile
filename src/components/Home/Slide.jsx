...
const Slide = ({ message, isActive }) => {
    ...
    return (
        <div className="w-full max-w-7xl mx-auto h-full flex-shrink-0 bg-white">
            <div className="max-w-6xl mx-auto h-full grid grid-cols-1 lg:grid-cols-2 lg:gap-12 gap-6 items-center px-6 md:px-16 lg:px-20">
                ...
                <div className={`relative flex justify-center lg:justify-start transition-all duration-1000 transform ${isActive ? 'translate-x-0 opacity-100 scale-100' : 'lg:translate-x-full translate-y-10 opacity-0 scale-95'}`}>
                    <div className="relative group mx-auto lg:mx-0">
                        ...
                        <div className={`absolute -bottom-3 -right-4 px-4 py-2 bg-white rounded-2xl shadow-xl border border-gray-100 transition-all duration-1000 delay-500 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                            <p className="text-[#2a9fd8] italic text-sm md:text-lg font-bold">
                                {message.type === 'CEO' ? 'CEO & FOUNDER' : 'CTO & CO-FOUNDER'}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                {/* Right Side: Content */}
                <div className={`flex flex-col justify-center text-center lg:text-left transition-all lg:py-4 py-0 duration-1000 transform ${isActive ? 'translate-x-0 opacity-100' : 'lg:translate-x-full translate-y-10 opacity-0'}`}>
                    {/* Title with animated underline */}
                    <div className="lg:pb-2 pb-1">
                    ...
                    <div className="lg:space-y-6 space-y-3">
                        {message.content.map((paragraph, index) => (
                            <p
                                key={index}
                                className={`text-gray-600 text-sm md:text-base leading-relaxed transition-all duration-700 delay-${300 + (index * 100)} ${isActive
                    ...
                    <div className={`pt-4 lg:pt-2 border-gray-100 transition-all duration-700 delay-700 ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
